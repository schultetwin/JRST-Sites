...
class MailhandlerAuthenticateTokenauth extends MailhandlerAuthenticate {
  ...
  public function authenticate(&$message, $mailbox) {
    if (module_exists('tokenauth')) {
      ...
      elseif (function_exists('mailalias_user') && ($from_user = mailhandler_user_load_alias($fromaddress, $target_node, $item['mailbox'])) && strpos($header->to[0]->mailbox, tokenauth_get_token($from_user->uid)) !== FALSE) {
